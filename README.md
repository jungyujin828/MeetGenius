...
| 팀원     | 소감 |
|-----------|--------|
|정찬호 🐨| 마지막 2주가 너무 빡빡했습니다. 느리긴 했지만 결과적으로 성공적인 구현 및 시연을 해서 가장 보람 있었던 순간인 것 같습니다. 또한 웹단의 플로우를 잘 알지 못하고 시작한게 많이 아쉬웠다. 팀장과 PM을 동시에 맡기에는 타 개발을 동시에 진행하기에 살짝 힘든 느낌이 들었다. 도커 desktop을 모두 깔아두고, 인프라가 gitlab에 docker 환경까지 설정해둬서 개발자들이 받아서 그냥 바로 사용하는 식으로 하는게 어떘을까 생각이 들었습니다. |
|박가연 🐥| IoT 기술을 활용한 프로젝트는 처음이라서 신기했고 좋은 경험이었다.웹과 젯슨 사이에 주고 받아야하는 데이터에 대한 변동 사항이 많아서 어려웠지만, FastAPI로 엔드포인트를 작성하며 데이터 파이프라인을 구축하는 과정이 흥미로웠다. 데이터 담당을 맡았지만 기술 지식에 대한 한계를 많이 느꼈다. | 
|연재환 🤖| ARM 아키텍처 기반의 Jetson Orin Nano 초기 세팅이 매우 까다로웠으나, 공식 문서 및 관련 github issue들을 확인해가며 개발을 위한 세팅을 완료한 점이 보람찼다. LLM모델의 할루시네이션의 원인(Lost in the Middle)을 찾고, 해결하는 과정이 보람찼다. 마지막 주차 Jetson과 Web을 연결하는 과정에서 발생한 버그와 수정 과정에서의 Side-Effect 해결이 힘들었다. 다음 프로젝트에서는 개발 단계에서 파트 간, 팀원 간 주기적인 소통 / 진행 사항 공유 그리고 유기적인 역할 분담에 힘써야겠다. |
|김근휘 🐶| 명확한 설계의 부재로 중간에 코드를 엎어야하는 상황에 마주했다. 급하게 재설계하여 위기를 넘기고 기능을 구현했지만, 설계의 중요성을 느꼈다.
스트리밍 기능을 구현하기 위해 소켓과 SSE, 폴링을 두고 저울질 했다. 최소한의 비용으로 기능을 구현하고자 했으며, 각각의 기술을 썼을 때 기능을 모두 구현할 수 있는가? 웹소켓 또는 SSE를 썼을 떄 또는 쓰지 않았을 때 추가적으로 고려하고 구현해야하는 부분을 고려했다. 최종적으로 최소한의 비용으로 동일한 성능과 기능을 구현해서 보람찼다. 다음 프로젝트에서는 팀원들과 기획 설계 개발 모든 부분에서 동일한 이해도를 갖게 할 것, 기능이 겉보기에 동작한다고 코드리뷰를 소홀히 하지 않을 것설계단계에서 구현방법에 관하여 최대한 많이 고민할 것을 최대한 지켜야겠다.|
|장인영 🐹| 중간에 인력난으로 프론트엔드 개발에 투입되었는데, 새로운 걸 하게 되어 재미있었다. 프론트의 코드 리팩토링을 작업하면서, 기존의 기능에 문제 없이 간결화에 성공했다는 점이 보람있었다. 다음 프로젝트에서도 백엔드를 맡았기에, 프론트엔드와의 데이터 서빙을 어떻게 할지 생각해보고, 프론트엔드와 백엔드에 통신하고 있는 방식 등을 함께 공부하고 학습하겠습니다. 또한 적합한 기술을 찾아 적용하기 위해 노력도록 하겠습니다|
|정유진 🐇| |
